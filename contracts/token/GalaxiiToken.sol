...
contract Wemp is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 600 * 10**12 * 10**9;
 ...
 mapping (address => bool) private _isExcluded;
    ...
    string private constant _name = 'WEMP';
...
function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
    ...
    uint256 rcharityfee=0;
        ...
        _reflectFee(rFee.sub(rcharityfee), tFee);
  ...
  for (uint256 i = 0; i < _excluded.length; i++) {
    ...
    function swtchcharityupdate(bool _charityenabled,uint256 value) external onlyOwner {
        require(value <= 100, "TaxFee exceed 100");
    ...
    function updateCharityWallet(address _charitywallet) external onlyOwner {
         require(_charitywallet != address(0), "new charityWallet is the zero address");
    ...
    function removeAllTAx() private {
        if(charitypercent==0 && burnpercent==0 && rewardpercent==0) return;
